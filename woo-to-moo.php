...
/*
 * Plugin Name: Woo to Moo
 * Version:     1.0
 * Plugin URI:  https://wordpress.org/plugins/woo-to-moo/
...
function wootomoo_load_textdomain() {
    load_plugin_textdomain( 'woo-to-moo', false, dirname( plugin_basename( __FILE__ ) ) . '/languages/' );
}
...
function wootomoo_validate_url_key( $input ) {
    ...
    if( null == $input || null == $url ) {
        add_settings_error( WOOTOMOO_URL_KEY, 'error', __('Invalid URL', 'woo-to-moo') );
...
function wootomoo_validate_token_key( $input ) {
    ...
    if( null == $input || strlen( $key ) != 32 ) {
        add_settings_error( WOOTOMOO_TOKEN_KEY, 'error', __('The token must have 32 characters', 'woo-to-moo') );
