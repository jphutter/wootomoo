...
function wootomoo_payment_complete_action( $order_id ) {
    ...
    if( $account_id >= 0 ) {
        ...
        $to = $user_email;
        $subject = __( 'Here are your connection data', 'woo-to-moo' );
        $heading = __( 'Here are your connection data', 'woo-to-moo' );

        $html_message = '<h2>' . __( 'Here are your connection data', 'woo-to-moo' ) . '</h2>';
        ...
        $html_message .= '<tfoot>';
        $html_message .= '<tr>';
        $html_message .= '<th class="td" scope="row" colspan="1" style="text-align: left; border-top-width: 4px; color: #636363; border: 1px solid #e5e5e5; padding: 12px;">' . __( 'Web site', 'woo-to-moo' ) . ' :</th>';
        ...
        $html_message .= ('<td class="td" style="text-align: left; color: #636363; border: 1px solid #e5e5e5; padding: 12px;">' . (is_null($user_password) ? __( 'your usual password', 'woo-to-moo' ) : $user_password) . '</td>');
        $html_message .= '</tr>';
        $html_message .= '</tfoot>';
        $html_message .= '</table>';
        $html_message .= '</div>';
        ...
        if( count($success) > 1 )
            $html_message .= __( 'For the following courses', 'woo-to-moo' ) . '</h2>';
        else
            $html_message .= __( 'For the following course', 'woo-to-moo' ) . '</h2>';
        $html_message .= '<div style="margin-bottom: 40px;">';
        $html_message .= '<table class="td" cellspacing="0" cellpadding="6" style="width: 100%; font-family: \"Helvetica Neue\", Helvetica, Roboto, Arial, sans-serif; color: #636363; border: 1px solid #e5e5e5;" border="1">';
        $html_message .= '<tfoot>';
